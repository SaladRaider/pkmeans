...
void PKMeansLowMem::run (int numClusters, int numThreads,
                         std::string inFilename,
                         std::string assignmentsOut,
                         std::string clustersOut) {
    printf ("Running pkmeans (low memory) with args (k=%d, t=%d, i=%s, a=%s, c=%s)\n",
...
void PKMeansLowMem::initClusters (int numClusters) {
    ...
    for (size_t k = 1; k < kMax; k++) {
        ...
        for (x = 0; x < distributions.size (); x++)
            weightedP[x] = Distribution<float>::emd (distributions[x], distributions[getCluster (x)]);
        weightedP *= weightedP;
...
size_t PKMeansLowMem::findClosestInitCluster (size_t x) {
    ...
    if (Distribution<float>::emd (distributions[x], clusters[cx]) <
        Distribution<float>::emd (distributions[x], clusters[clusters.size () - 1])) {
        return cx;
    }
    return clusters.size () - 1;
}

size_t PKMeansLowMem::getCluster (size_t x) {
    return clusterMap[x];
}

size_t PKMeansLowMem::findClosestCluster (size_t x) {
    size_t closestIdx = 0;
    float dist = Distribution<float>::emd (distributions[x],
                                             clusters[0]);
    float newDist;
    for (size_t i = 1; i < clusters.size (); i++) {
        newDist = Distribution<float>::emd (distributions[x],
                                             clusters[i]);
        if (newDist < dist) {
            closestIdx = i;
            dist = newDist;
